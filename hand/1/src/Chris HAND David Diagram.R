...
ss=raster(name_ss)
plot(ss,main = "ss")
#THIS IS NOT CURRENTLY IN USE. It can be input into areaD8 if needed

# d8 flow directions
system(paste("mpiexec -n 8 d8flowdir -p ",name_p," -sd8 ",name_sd8," -fel ",name_fel,sep = ""))
p=raster(name_p)
...
system(paste("mpiexec -n 8 dinfflowdir -ang ",name_ang," -slp ",name_slp," -fel ",name_fel,sep = ""))
ang=raster(name_ang)
plot(ang,main = "ang")
slp=raster(name_slp)
plot(slp,main = "slp")
...
# threshold
system(paste("mpiexec -n 8 threshold -ssa ",name_ad8," -src ",name_src," -thresh 40.55",sep = ""))
src=raster(name_src)
plot(src,main = "src")

###
# distance down
system(paste("mpiexec -n 8 dinfdistdown -ang ",name_ang," -fel ",name_fel," -src ",name_src," -m ave v -dd ",name_dd,sep = ""))
dd=raster(name_dd)
plot(dd,main = "dd")
